/*!
 * CUSTOM TO FIX Android Issue
 * https://github.com/mbenford/ngTagsInput/pull/692
 * ngTagsInput v3.0.0
...
tagsInput.directive('tagsInput', ["$timeout", "$document", "$window", "tagsInputConfig", "tiUtil", function($timeout, $document, $window, tagsInputConfig, tiUtil) {
    ...
    return {
        ...
        link: function(scope, element, attrs, ngModelCtrl) {
            ...
            scope.eventHandlers = {
                input: {
                    keydown: ((window.cordova && ionic.Platform.isAndroid()) ? (function($event) {}) : (function($event) {
                        events.trigger('input-keydown', $event);
                    })),
                    onchange: (!(window.cordova && ionic.Platform.isAndroid()) ? (function($event) {}) : (function($event) {
                        events.trigger('input-keydown', $event);
                    })),
                    focus: function() {
                        if (scope.hasFocus) {
            ...
            var extraEventKeyDown = function(event, newTagText) { 
                return { text: newTagText, key: event.keyCode }; 
            };
            var processEventKeyDown = function(event) { 
                return event;
            };

            if (window.cordova && ionic.Platform.isAndroid()) {
                processEventKeyDown = function(event) {
                    return { keyCode: null, preventDefault: function() { } }
                };
                extraEventKeyDown = function(event, newTagText) {
                    if (newTagText.length > 0 && newTagText[newTagText.length -1] == ',')
                        return { text: newTagText.slice(0, -1), key: KEYS.comma }; 
                    else
                        return { text: newTagText, key: event.keyCode }; 
                    // var newValue = event.target.value;
                    // if (newValue.length > 0 && newValue[newValue.length -1] == ',')
                    //     return { text: newValue.slice(0, -1), key: KEYS.comma }; 
                    // else if (newTagText.length > 0 && newTagText[newTagText.length -1] == ',')
                    //     return { text: newTagText.slice(0, -1), key: KEYS.comma }; 
                    // else {
                    //     newValue = event.target.nextSibling.textContent;
                    //     if (newValue.length > 0 && newValue[newValue.length -1] == ',')
                    //         return { text: newValue.slice(0, -1), key: KEYS.comma }; 
                    //     else
                    //         return { text: newTagText, key: event.keyCode }; 
            ...
            events
                ...
                .on('input-keydown', function(event) {
                    event = processEventKeyDown(event);
                    ...
                    var result = extraEventKeyDown(event, newTagText);
                    newTagText = result.text;
                    key = result.key;
...
tagsInput.run(["$templateCache", function($templateCache) {
    ...
    "<div class=\"host\" tabindex=\"-1\" ng-click=\"eventHandlers.host.click()\" ti-transclude-append><div class=\"tags\" ng-class=\"{focused: hasFocus}\"><ul class=\"tag-list\"><li class=\"tag-item\" ng-repeat=\"tag in tagList.items track by track(tag)\" ng-class=\"{ selected: tag == tagList.selected }\" ng-click=\"eventHandlers.tag.click(tag)\"><ti-tag-item data=\"::tag\"></ti-tag-item></li></ul><input class=\"input\" autocomplete=\"off\" autocorrect=\"off\" autocapitalize=\"off\" spellcheck=\"false\" ng-model=\"newTag.text\" ng-model-options=\"{getterSetter: true}\" ng-change=\"eventHandlers.input.onchange($event)\" ng-keydown=\"eventHandlers.input.keydown($event)\" ng-focus=\"eventHandlers.input.focus($event)\" ng-blur=\"eventHandlers.input.blur($event)\" ng-paste=\"eventHandlers.input.paste($event)\" ng-trim=\"false\" ng-class=\"{'invalid-tag': newTag.invalid}\" ng-disabled=\"disabled\" ti-bind-attrs=\"{type: options.type, placeholder: options.placeholder, tabindex: options.tabindex, spellcheck: options.spellcheck}\" ti-autosize></div></div>"
  );
